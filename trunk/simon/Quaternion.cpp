...
*
* Anmerkungen: Zu möglichen Konventionen siehe "Ken Shoemake: Euler Angle Conversion", GraphicsGems IV, Kapitel III.5,
* Seiten 222-229. 
*
...
/**
* \brief Konstruktor, der als Input die Eulerwinkel hat (im Bogenmaß)
* \param float phix, phiy, phiz (Eulerwinkel im Bogenmaß)
*
* Konvention: Rz*Ry*Rx !!! (s.o.)
* Angepasster Code aus OpenSG Quaternionen... 
*
* Idee: Ein Quaternion repräsentiert eine Rotation durch
* [cos(phi/2),sin(phi/2)*Rotationsachse]. Daher kann man die Eulerwinkel in Quaternionen umrechnen, indem man für
* jede Euler-Rotationsachse ein eigenes Quaternion erstellt, und diese Quaternionen multipliziert.
* Dabei ist für die Reihenfolge der Rotationen zu beachten: Q1*Q2*Q3 bedeutet, das zunächst die Q3-Rotation angewendet 
...
/**
* \brief Gibt gewünschtes Element des Quaternions zurück
*/
...
/**
* \brief Identitäts-Quaternion für die Multiplikation
*/
...
/**
* \brief Gibt das inverse Quaternion zurück
...
* \return Gibt die Magnitude (Norm) zurück (float)
*/
...
/**
* \brief Normalisiert die Rotationsachse eines Orientierungs-Quaternions (noch nicht sicher ob nötig)
*/
...
* \param float x, y, z die Eulerwinkel in Bogenmaß
*/
...
/**
* \brief Gibt die einzelnen 4 Komponenten des Quaternions zurück
* \return Setzt die Werte in die übergebenen Referenzen
*/
...
*
* Idee:
* getRotationMatrix liefert eine Matrix. Diese Matrix ist eben jene, die man erhält, wenn man drei
* Rotationsmatrizen (jeweils um z, y, x-Achse) in der Reihenfolge Rz*Ry*Rx multipliziert. atan2(b,a) rechnet
* intern atan(b/a). Dementsprechend werden aus der Matrix Werte so an atan2 übergeben, das b/a im Endeffekt 
* sinx/cosx=tanx (bzw. tany, tanz) ergibt.
* a und b werden so aus der Matrix gewählt, dass sich cos(phiy) immer wegkürzt. Daher muss der cos(phiy)=0 abgefangen 
* werden (else-Zweig). Dies ist für rotationY nicht möglich, daher muss hier cos(phiy) anders berechnet werden. Hierzu wird 
...
* geschicktes Vorgehen diese Sinus-/Cosinuswerte rückrechnen. Dabei werden die Quaternionenwerte so geschickt zusammen-
* gerechnet, das sich aufgrund der Trigonometrischen Gesetzmässigkeiten im Endeffekt die Einträge der Rotationsmatrix 
* Rz*Ry*Rx ergeben.
* 
* Diesen Code haben wir nicht bis ins letzte Detail nachvollzogen, ihn allerdings durch ausführliche Vergleichstests bestätigt.
*
* Dieser Code wurde aus OpenSG übernommen. OpenSG rechnet anscheinend mit transponierten Matrizen, daher wird die
* Ergebnismatrix "zurück"-transponiert.
*/
